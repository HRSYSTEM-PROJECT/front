...
export default function PlanPage() {
  ...
  // Función para traer el plan actual desde el backend
  const fetchCurrentPlan = useCallback(async () => {
    ...
    try {
      ...
      if (!planPrice || isNaN(planPrice)) {
        const matchedPlan = plans.find((p) => p.name === suscripcion?.plan?.name);
  ...
  }, [companyId, getToken, plans]);

  // Cargar planes y suscripción al inicio
  useEffect(() => {
    const fetchData = async () => {
      if (!userId) return;
  ...
  // Actualiza precio si cambia el plan
  useEffect(() => {
    const plan = plans.find((p) => p.name === currentPlan);
    if (plan) setPrice(Number(plan.price));
  }, [currentPlan, plans]);
  ...
  return (
    <div className="container mx-auto px-4 sm:px-6 py-4 text-start max-w-full overflow-x-hidden">
      ...
      <div className="mb-10 bg-white border border-gray-200 shadow-lg rounded-xl p-6">
        <h2 className="text-2xl font-semibold text-gray-700 mb-3 flex items-center">
          Tu Plan Actual
        </h2>
        <div className="flex justify-between items-center">
        ...
        <p className="mt-4 text-sm text-gray-500">
          ¿Querés dejar de ser parte{" "}
          <span className="font-semibold text-[#083E96]">HR SYSTEM</span>?{" "}
          <button
            onClick={() => console.log("Desuscribirse clickeado")}
